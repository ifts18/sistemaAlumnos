	...
	if($action == 'ajax'){
		...
		//consulta principal para recuperar los datos
                ...
                if($method == 'buscar'){
                 ...
                 $sql = "SELECT a.idAlumno, a.DNI , a.Apellido, a.Nombre , m.Descripcion as Materia, 
						if(am.FechaFirma is not null, 'Si ', 'No') as Firmada , 
						case when (select mfa.aprobado from mesa_final_alumno mfa where am.idAlumnoMateria = mfa.IdAlumnoMateria LIMIT 1 ) = 1 
						or ae.idAlumnoEquivalencia is not null then 'Si' else 'No' end as Aprobada, 
						(select mfa.nota from mesa_final_alumno mfa where am.idAlumnoMateria = mfa.IdAlumnoMateria LIMIT 1 ) as Nota,
                                                (select mfa.IdMesaFinalAlumno from mesa_final_alumno mfa where am.idAlumnoMateria = mfa.IdAlumnoMateria LIMIT 1 ) as IdMesaFinalAlumno
                                                FROM alumnos a 
                    ...
                    $count_query = mysqli_query(dbconnect(),"SELECT count(*) AS numrows  FROM (
										...
										if(am.FechaFirma is not null, 'Si ', 'No') as Firmada , 
										case when (select 1 from mesa_final_alumno mfa where am.idAlumnoMateria = mfa.IdAlumnoMateria LIMIT 1 ) = 1 
										or ae.idAlumnoEquivalencia is not null then 'Si' else 'No' end as Aprobada, 
										(select mfa.nota from mesa_final_alumno mfa where am.idAlumnoMateria = mfa.IdAlumnoMateria LIMIT 1 ) as Nota 
										FROM alumnos a 
										INNER JOIN alumno_materias am on am.IdAlumno = a.IdAlumno 
